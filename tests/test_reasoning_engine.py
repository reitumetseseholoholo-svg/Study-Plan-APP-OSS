...
class TestReasonQuestion:
    ...
    def test_wacc_explicit(self):
        r = reason_question(
            ...
            template_inputs={"equity": 60, "debt": 40, "cost_equity": 0.12, "cost_debt": 0.042, "tax_rate": 0.30},
        )
        assert r.has_result
    ...
    def test_payback(self):
        r = reason_question(
            "Payback", template_ref="fm.payback", template_inputs={"initial": 100, "cashflows": [30, 40, 50]}
        )
        assert r.has_result
        assert abs(r.final_result - 2.6) < 0.01

    def test_gearing(self):
        r = reason_question("Gearing", template_ref="fm.gearing", template_inputs={"debt": 40, "equity": 60})
        assert r.has_result
    ...
    def test_eoq(self):
        r = reason_question(
            "EOQ",
            template_ref="fm.eoq",
            template_inputs={"annual_demand": 10000, "ordering_cost": 50, "holding_cost_per_unit": 2},
        )
        assert r.has_result
        assert abs(r.final_result - 707.1) < 1.0

    def test_ccc(self):
        r = reason_question("CCC", template_ref="fm.ccc", template_inputs={"dio": 40, "dso": 30, "dpo": 20})
        assert r.has_result
        assert abs(r.final_result - 50) < 0.01

    def test_eps(self):
        r = reason_question(
            "EPS", template_ref="fm.eps", template_inputs={"profit_after_tax": 2000000, "number_of_shares": 500000}
        )
        assert r.has_result
    ...
    def test_arr(self):
        r = reason_question(
            "ARR", template_ref="fm.arr", template_inputs={"average_profit": 15000, "initial_investment": 100000}
        )
        assert r.has_result
        assert abs(r.final_result - 0.30) < 0.01

    def test_pe_ratio(self):
        r = reason_question("P/E ratio", template_ref="fm.pe_ratio", template_inputs={"market_price": 10.0, "eps": 2.0})
        assert r.has_result
        assert abs(r.final_result - 5.0) < 0.001

    def test_roe(self):
        r = reason_question(
            "ROE", template_ref="fm.roe", template_inputs={"profit_after_tax": 500000, "equity": 2000000}
        )
        assert r.has_result
    ...
    def test_terp(self):
        r = reason_question(
            ...
            template_inputs={"cum_rights_price": 2.00, "issue_price": 1.50, "rights_ratio_n": 4},
        )
        assert r.has_result
    ...
    def test_roce(self):
        r = reason_question("ROCE", template_ref="fm.roce", template_inputs={"pbit": 50000, "capital_employed": 250000})
        assert r.has_result
    ...
    def test_asset_beta(self):
        r = reason_question(
            ...
            template_inputs={"equity_beta": 1.2, "market_value_debt": 40, "market_value_equity": 60, "tax_rate": 0.30},
        )
        assert r.has_result
...
class TestDependencyChaining:
    def test_irr_uses_npv(self):
        ...
        r = reason_question(
            "IRR question", template_ref="fm.irr", template_inputs={"cashflows": [60, 60], "initial": 100}
        )
        assert r.has_result
    ...
    def test_dividend_cover_uses_eps(self):
        ...
        r = reason_question("Div cover", template_ref="fm.dividend_cover", template_inputs={"dividend_per_share": 0.20})
        assert r.has_result
        concept_ids = [e.concept_id for e in r.execution]
    ...
    def test_dividend_cover_skips_when_eps_given(self):
        ...
        r = reason_question(
            "Div cover", template_ref="fm.dividend_cover", template_inputs={"eps": 0.40, "dividend_per_share": 0.20}
        )
        assert r.has_result
    ...
    def test_equity_beta_plan_includes_asset_beta(self):
        ...
        r = reason_question(
            ...
            template_inputs={"market_value_debt": 40, "market_value_equity": 60, "tax_rate": 0.30},
        )
    ...
    def test_equity_beta_skips_when_asset_beta_given(self):
        ...
        r = reason_question(
            "Equity beta",
            template_ref="fm.equity_beta",
            template_inputs={"asset_beta": 0.8, "market_value_debt": 40, "market_value_equity": 60, "tax_rate": 0.30},
        )
        assert r.has_result
    ...
    def test_dependency_order_preserved(self):
        ...
        r = reason_question("IRR q", template_ref="fm.irr", template_inputs={"cashflows": [60, 60], "initial": 100})
...
class TestExecutionEdgeCases:
    def test_nan_result_handling(self):
        ...
        r = reason_question("Bad inputs", template_ref="fm.npv", template_inputs={"cashflows": [], "rate": -1})
        # Should not crash
        assert r.execution
    ...
    def test_all_steps_recorded(self):
        r = reason_question("NPV", template_ref="fm.npv", template_inputs={"cashflows": [100], "rate": 0.10})
...
class TestTraceOutput:
    def test_to_dict_serializable(self):
        r = reason_question(
            "NPV", template_ref="fm.npv", template_inputs={"cashflows": [100, 200], "rate": 0.10, "initial": 50}
        )
        d = r.to_dict()
    ...
    def test_confidence_partial(self):
        ...
        r = reason_question("WACC", template_ref="fm.wacc", template_inputs={"equity": 60, "debt": 40})
...
class TestDiagnosticIntegration:
    def test_diagnostic_attached(self):
        r = reason_question(
            "NPV question", template_ref="fm.npv", template_inputs={"cashflows": [100, 200], "rate": 0.10}
        )
    ...
    def test_diagnostic_concept_ids(self):
        r = reason_question(
            "WACC question",
            template_ref="fm.wacc",
            template_inputs={"equity": 60, "debt": 40, "cost_equity": 0.12, "cost_debt": 0.06, "tax_rate": 0.30},
        )
...
class TestBuildInputsFor:
    ...
    def test_explicit_overrides(self):
        inputs = _build_inputs_for(
            "fm.capm", "question", "fm.capm", {"risk_free": 0.03, "beta": 1.2, "market_return": 0.10}, {}
        )
        assert inputs.get("risk_free") == 0.03

    def test_intermed_forwarding(self):
        inputs = _build_inputs_for(
            "fm.equity_beta", "question", None, None, {"asset_beta": 0.8, "market_value_debt": 40}
        )
        assert inputs.get("asset_beta") == 0.8

    def test_filters_unexpected_keys(self):
        inputs = _build_inputs_for(
            "fm.capm", "question", None, None, {"risk_free": 0.03, "beta": 1.2, "not_a_param": 99}
        )
...
class TestMultiPathFallback:
    ...
    def test_no_fallback_when_no_alternative(self):
        ...
        r = reason_question("Payback fail", template_ref="fm.payback", template_inputs={})
        assert len(r.execution) == 1
        assert not r.execution[0].success
    ...
    def test_fallback_not_used_when_original_succeeds(self):
        ...
        r = reason_question(
            "NPV", template_ref="fm.npv", template_inputs={"cashflows": [100], "rate": 0.10, "initial": 50}
        )
...
class TestGapAnalysis:
    def test_capm_discovered_when_cost_equity_missing_and_capm_inputs_available(self):
        ...
        assert "fm.capm" in concept_ids, f"CAPM should be auto-discovered, got {concept_ids}"
    ...
    def test_no_gap_added_when_provider_inputs_unavailable(self):
        ...
        concept_ids = [p.concept_id for p in plan]
        assert "fm.capm" not in concept_ids, f"CAPM should NOT be added without its inputs, got {concept_ids}"

    def test_plug_input_gaps_noop_on_simple_concept(self):
        """Gap analysis should not modify a plan that has no missing inputs."""
        plan = _compile_plan("fm.npv", BUILTIN_CONCEPTS, "NPV 10% 100", {})
        original = list(plan)
...
class TestWeightedConfidence:
    ...
    def test_failed_deps_lower_but_not_zero_confidence(self):
        ...
        r = reason_question("WACC partial", template_ref="fm.wacc", template_inputs={"equity": 60, "debt": 40})
        assert 0 < r.confidence < 1.0
    ...
    def test_compute_confidence_all_success(self):
        recs = [
            ExecutionRecord(concept_id="fm.npv", success=True, input_source_quality=1.0),
        ]
    ...
    def test_compute_confidence_mixed_quality(self):
        ...
        recs = [
            ExecutionRecord(concept_id="fm.a", success=True, input_source_quality=0.70),
        ]
    ...
    def test_compute_confidence_with_failures(self):
        ...
        recs = [
            ExecutionRecord(concept_id="fm.a", success=False, input_source_quality=0.70),
            ExecutionRecord(concept_id="fm.b", success=True, input_source_quality=1.0),
        ]
        c = _compute_confidence(recs)
    ...
    def test_compute_confidence_skipped_ignored(self):
        recs = [
            ExecutionRecord(concept_id="fm.a", success=False, skipped=True, input_source_quality=0.0),
            ExecutionRecord(concept_id="fm.b", success=True, input_source_quality=1.0),
        ]
