"""Return on equity concept template."""
...
from studyplan.numerical_solver import solve_roe
from studyplan.domain_reasoning.templates import FormulaTemplate


class RoeTemplate(FormulaTemplate):
    def __init__(self) -> None:
        super().__init__("fm.roe", solve_roe, version="1.0.0")

    def solve(self, inputs: dict[str, Any]) -> dict[str, Any]:
        pat = float(inputs.get("profit_after_tax", 0))
        eq = float(inputs.get("equity", 1))
        result = self._solver(pat, eq)
            ...
            if sid == "roe":
                ...
                if sv is not None:
                    ...
                    if truth_val is not None and float(sv) != float(truth_val):
                        tags.append("equity_error" if float(sv) > float(truth_val) else "profit_error")
        return tags
