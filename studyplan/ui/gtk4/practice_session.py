...
from studyplan.contracts import TutorAssessmentSubmission, TutorAssessmentResult
...
class PracticeSessionWindow(Gtk.Box):
    ...
    def _create_loop_state(self):
        ...
        session_state = TutorSessionState(
            session_id="practice_session", module="ACCA", topic="General", mode="guided_practice", loop_phase="practice"
        )
    ...
    def _show_feedback(self, message: str, message_type: str = "info"):
        """Show feedback message with appropriate styling via CSS classes."""
        css_map = {"info": "status-info", "warning": "status-warn", "error": "status-bad", "success": "status-ok"}
        css_class = css_map.get(message_type, "status-info")
        for cls in css_map.values():
            try:
                self.feedback_display.remove_css_class(cls)
            except Exception:
                pass
        try:
            self.feedback_display.add_css_class(css_class)
        except Exception:
            pass
        markup = f"<b>{message}</b>"
