...
class MisconceptionLibrary:
    ...
    def diagnose_error(
        ...
        if "sign_error" in error_tags or (
            user_answer and expected_answer and _is_likely_sign_error(user_answer, expected_answer)
        ):
            misconception = None
    ...
    def get_remediation_steps(misconception: Misconception) -> list[str]:
        ...
        return [
            ...
            "Reflect: How does this change your understanding?",
        ]
    ...
    def detect_pattern(self) -> tuple[str, float] | None:
        ...
        for error in self.error_history[-5:]:  # Last 5 errors
            if error.misconception:
                misconception_counts[error.misconception.id] = misconception_counts.get(error.misconception.id, 0) + 1
