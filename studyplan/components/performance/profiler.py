    ...
    def _generate_recommendations(self, operation_name: str, stats: Dict[str, float]) -> List[str]:
        ...
        if stats["avg_duration_ms"] > 100.0:
            recommendations.append(
                f"Operation '{operation_name}' has high average latency ({stats['avg_duration_ms']:.2f}ms). Consider caching or optimization."
            )

        # Check for high p95 latency
        if stats["p95_duration_ms"] > 500.0:
            recommendations.append(
                f"Operation '{operation_name}' has high p95 latency ({stats['p95_duration_ms']:.2f}ms). Investigate outliers."
            )

        # Check for low success rate
        if stats["success_rate"] < 0.95:
            recommendations.append(
                f"Operation '{operation_name}' has low success rate ({stats['success_rate']:.2%}). Check for error patterns."
            )

        # Check for high error count
        if stats["error_count"] > 10:
            recommendations.append(
                f"Operation '{operation_name}' has high error count ({stats['error_count']}). Review error logs."
            )
...
PERFORMANCE_PROFILER_CONFIG_SCHEMA = {
    "enabled": {"type": "boolean", "default": True, "description": "Enable performance profiling"},
    "alert_thresholds": {
        "type": "object",
        "properties": {
    ...
    "alert_window_size": {"type": "integer", "default": 50, "description": "Number of alerts to keep in memory"},
}
